...
app.use("/dist/ui",express.static(path.join(__dirname,"../dist/ui")));
console.log(path.join(__dirname,"../dist/ui"))
// 서버 띄우기
...
app.get("/register", function(req:express.Request,res:express.Response){
  res.render('register', {"responseId":req.query.responseId, "result":""});
});
...
app.post("/register", function(req:express.Request,res:express.Response){
  ...
  .then(()=>{
    ...
    const user:User = userdb.addNewUser(register_context);
    res.send({data:user});
  })
  .catch(result => res.send({data:result.message}));
});
